...
class Protonet(nn.Module):
    ...
    def loss(self, sample):
        ...
        z1 = {'data': z, 'n_class': n_class, 'n_support': n_support, 'n_query': n_query}
        z_proto = self.encoder[1].forward(z1)
...
class instance_attention_layer(nn.Module):
    ...
    # def __init__(self, feature_dim, n_class, n_support, n_query):
    def __init__(self, feature_dim):
    ...
    def forward(self, x):
        ...
        # support = x[:self.n_class*self.n_support].view(self.n_class, self.n_support, self.feature_dim).unsqueeze(0).expand(self.n_query*self.n_class, -1, -1, -1)
        # support_for_att = self.fc(support)
        # query_for_att = self.fc(x[self.n_class*self.n_support:].unsqueeze(1).unsqueeze(2).expand(-1, self.n_class, self.n_support, -1))
        # ins_att_score = F.softmax(torch.tanh(support_for_att * query_for_att).sum(-1), dim=-1)
        # support_proto = (support * ins_att_score.unsqueeze(3).expand(-1, -1, -1, self.feature_dim)).sum(2)
...
def load_Protonet(**kwargs):     # Do we need add a Neural network layer after feature attention layer?
    ...
    # def instance_lay(in_dim, n_class, n_support, n_query):
    #     return  nn.Sequential(
    #         instance_attention_layer(in_dim, n_class, n_support, n_query)
    #     )
    ...
    encoder = nn.Sequential(
        ...
        # instance_lay(feature_dim, n_class, n_support, n_query)
